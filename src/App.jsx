...
import { 
  ...
  RefreshCw, Stethoscope, AlertOctagon, XCircle, ScanLine 
} from 'lucide-react';
...
// --- IMAGE COMPRESSOR ENGINE (PREVENTS MEMORY CRASHES) ---
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Resize to max 1024px (Plenty for AI, small enough for mobile memory)
...
const LOCALE_DATA = {
  ...
  // ... (Other languages kept implicit for brevity, they work identically via the key system) ...
  // Assume other languages (ta, hi, te, kn, ml) are present as per previous versions. 
  // If you need them pasted explicitly again, let me know, but this keeps the file size manageable for copy-paste.
  ta: {
...
-- Scanned with CocoMed`.trim();
  ...
  const analyzeMedicine = async (base64Image) => {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        // FAIL-SAFE CHECK 1: Explicit Error from AI
        ...
        const cleanData = sanitizeScanData(parsedResult);
        // FAIL-SAFE CHECK 2: Data Integrity
      ...
      console.error(error);
      // FAIL-SAFE ACTION: Show Error Banner on Home Screen instead of crashing
      setApiError("This medication is not supported in our search or the image was unclear. Please try again with a clearer photo.");
  ...
  const handleFileSelect = async (event) => {
    ...
    if (file) {
      // Compressing image before analysis to prevent memory crash
      try {
          const compressedBase64 = await compressImage(file);
          // compressImage returns full data url "data:image/jpeg;base64,..."
          // we need to strip the prefix for the API call
  ...
  const compressImage = (file) => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          const canvas = document.createElement('canvas');
          const MAX_WIDTH = 800; // Reduced size for safety
          const scaleSize = MAX_WIDTH / img.width;
          canvas.width = MAX_WIDTH;
          canvas.height = img.height * scaleSize;
          const ctx = canvas.getContext('2d');
